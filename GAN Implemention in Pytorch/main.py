...
##  创建文件夹
os.makedirs("./images/gan/", exist_ok=True)  ## 记录训练过程的图片效果, 代表目前所在的目录.
...
#
## opt = parser.parse_args(args=[])                 ## 在colab中运行时，换为。
print(args)
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            *block(args.latent_dim, 128, normalize=False),  ## 线性变化将输入映射 100 to 128, 正则化, LeakyReLU
...
## ----------
## 进行多个epoch的训练
for epoch in range(args.n_epochs):  ## epoch:50
    for i, (imgs, _) in enumerate(dataloader):  ## imgs:(64, 1, 28, 28)     _:label(64)
        ...
        z = Variable(torch.randn(imgs.size(0), args.latent_dim)).to(device)  ## 随机生成一些噪声, 大小为(128, 100)
        ...
        ##
        z = Variable(torch.randn(imgs.size(0), args.latent_dim)).to(device) ## 得到随机噪声
        ...
        if (i + 1) % 100 == 0:
            print(
                ...
                % (epoch, args.n_epochs, i, len(dataloader), loss_D.item(), loss_G.item(), real_scores.data.mean(),
